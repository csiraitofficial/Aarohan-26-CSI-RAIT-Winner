...
#  (XGBoost predicts per-student; talking needs pair context)
...
def apply_talking_override(tracks: list):
    ...
    This is kept as a post-process because talking is relational.
    """
    for i in range(len(tracks)):
        for j in range(i + 1, len(tracks)):
            ...
            if math.hypot(ax - bx, ay - by) > TALKING_DIST:
                continue
            # Check opposite yaw stored in each track
...
class StudentTrack:
    ...
    def __init__(self, raw_box):
        ...
        self.head_yaw  = 0.0   # stored for talking override
...
def main():
    ...
    while True:
        ...
        # ── Feature extraction + XGBoost prediction 
        if frame_idx % SKIP_FRAMES == 0:
            for t in tracks:
                ...
                if feat is not None:
                    ...
                    t.confidence = conf
                    t.score      = score
                    # Store yaw for talking override
                    try:
                        t.head_yaw = float(feat[0, 5])
                    except Exception:
                        t.head_yaw = 0.0

            # Talking pair override (relational – needs all tracks)
